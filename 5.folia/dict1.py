...
def kisBetu_lista(str):
    """
    A sztringet kisbetűssé alakítja és listázza a szavakat.
    Meghívja az onlyAlpha függvényt.
    """
...
def rendezettLista(li):
    """
    Sorba rendez és a szóismétlődést megszűnteti.
    """
...
def wordCount(words, li):
    """
    Összehasonlítja a szólistát a szöveggel
    """
    d = {}
    for w in li: # végigmegy a szólista szavain
        cnt = 0             # indítja a számlálót
        for word in words:  # végigmegy a szöveg szavain
            if w == word:   # ha egyezés van, a számlálót növeli egyel
                cnt += 1
        d[w] = str(cnt)     # a szótár kulcsot elnevezi az adott szóra, az értéket beállítja a számláló értékére és sztringre alakítja
